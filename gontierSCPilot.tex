...
\begin{abstract}
There is a need to better characterize the perception of urban sound environments. Studies in psychoperception show that assessing properties about characteristic sources is important. To produce relevant noise maps the contribution of each source must be estimated from a sensor network. This paper presents a perceptual test on simulated sound scenes for which conjointed source properties are known. Results show that it is indeed feasible to predict perceptual source-specific quantities of interest from recordings, leading to possible automatic soundscape characterization.
